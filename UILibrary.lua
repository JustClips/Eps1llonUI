...
mainFrame.Size = UDim2.new(0, 650, 0, 360) -- height reduced from 420 to 360
mainFrame.Position = UDim2.new(0.5, -325, 0.5, -180) -- offset updated for new height
    ...
    headerFrame.InputBegan:Connect(function(input)
        ...
        then
            dragging, dragStart, startPos =
                true, input.Position, mainFrame.Position
...
sidebar.Size = UDim2.new(0, 140, 0, 280) -- height reduced from 340 to 280
...
contentFrame.Size = UDim2.new(0, 480, 0, 280) -- height reduced from 340 to 280
...
local DEFAULT_COLOR, ACTIVE_COLOR, DEFAULT_SIZE, ACTIVE_SIZE =
    Color3.fromRGB(210, 210, 210), Color3.fromRGB(255, 255, 255), 16, 18
...
local function setButtonActive(idx)
    for i, group in ipairs(sidebarButtons) do
        ...
        TweenService
            :Create(btn, TweenInfo.new(0.16, Enum.EasingStyle.Quad), {
...
function Eps1llonUI:AddButton(tab, opts)
    ...
    btn.Position = UDim2.new(
        ...
        opts.X or 20,
        0,
        opts.Y or (#section:GetChildren() - 1) * 42 + 15
    )
...
function Eps1llonUI:AddLabel(tab, opts)
    ...
    lbl.Position = UDim2.new(
        ...
        opts.X or 20,
        0,
        opts.Y or (#section:GetChildren() - 1) * 32 + 12
    )
...
function Eps1llonUI:AddToggle(tab, opts)
    ...
    container.Position = UDim2.new(
        ...
        opts.X or 20,
        0,
        opts.Y or (#section:GetChildren() - 1) * 44 + 12
    )
    container.BackgroundColor3 = opts.BackgroundColor3
    ...
    pillow.BackgroundColor3 = opts.Default and Color3.fromRGB(43, 110, 158)
    ...
    knob.Position = opts.Default and UDim2.new(1, -22, 0, 2)
    ...
    local function setToggle(val)
        value = val
        pillow.BackgroundColor3 = val and Color3.fromRGB(43, 110, 158)
            or Color3.fromRGB(44, 44, 47)
        TweenService
            :Create(
                knob,
                ...
                {
                    Position = val and UDim2.new(1, -22, 0, 2) or UDim2.new(
                        0,
                        2,
                        0,
                        2
    ...
    pillow.TouchTap:Connect(function()
        setToggle(not value)
    end)
    return container
end
...
function Eps1llonUI:AddSlider(tab, opts)
    ...
    frame.Position = UDim2.new(
        ...
        20,
        0,
        opts.Y or (#section:GetChildren() - 1) * 58 + 10
    )
    ...
    fill.Size = UDim2.new(
        ((opts.Default or opts.Min) - opts.Min) / (opts.Max - opts.Min),
        0,
        1,
        0
    )
    ...
    local function setSliderPos(x)
        ...
        local value = math.floor(
            (opts.Min or 0)
                + ((opts.Max or 100) - (opts.Min or 0)) * percent
                + 0.5
        )
...
local function animateObj(
    ...
    local scaleObj = obj:FindFirstChildOfClass('UIScale')
        or Instance.new('UIScale', obj)
    ...
    local tw2 = TweenService:Create(
        obj,
        TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
        { BackgroundTransparency = transpTo }
    )
    tw1:Play()
    tw2:Play()
...
minimizedButton.InputBegan:Connect(function(input)
    if
        input.UserInputType == Enum.UserInputType.MouseButton1 or isTouch(input)
    then
        draggingMini = true
